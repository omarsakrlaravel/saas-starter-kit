<div x-data="{ open: false }" class="flex h-full md:flex-1">
    <div class="hidden flex-1 space-x-8 h-full font-semibold md:flex">
        <a href="{{ route('dashboard') }}" wire:navigate class="inline-flex items-center px-1 pt-1 text-sm leading-5 transition duration-150 ease-in-out focus:outline-none border-b-2 border-transparent @if(Request::is('dashboard')){{ 'text-zinc-900' }}@else{{ 'text-zinc-800 hover:text-zinc-900' }}@endif">Dashboard</a>
        ...
        <div @click.away="open = false" class="flex relative items-center ml-3 h-full" x-data="{ open: false }">
            ...
            <div
                ...
                <div class="bg-white rounded-xl border shadow-md border-zinc-100" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                    <a href="{{ route('profile', auth()->user()->username) }}" wire:navigate class="block px-4 py-3 text-zinc-700 hover:text-zinc-800">
                    ...
                    <div class="py-1">
                        ...
                        @trial
                            <a href="{{ route('settings.subscription') }}" wire:navigate class="block px-4 py-2 text-sm leading-5 text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-none focus:bg-zinc-100 focus:text-zinc-900">Upgrade My Account</a>
                        @endtrial
                        ...
                        @endif
                        <a href="{{ route('profile', auth()->user()->username) }}" wire:navigate class="block px-4 py-2 text-sm leading-5 text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-none focus:bg-zinc-100 focus:text-zinc-900">My Profile</a>My Profile</a>
                        <a href="{{ route('settings') }}" wire:navigate class="block px-4 py-2 text-sm leading-5 text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-none focus:bg-zinc-100 focus:text-zinc-900">Settings</a>

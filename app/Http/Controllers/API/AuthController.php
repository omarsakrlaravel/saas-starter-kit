...
class AuthController extends Controller implements HasMiddleware
{
    ...
    public function login(Request $request): JsonResponse
    {
        ...
        $user = Auth::user();
        $expiresAt = now()->addMinutes((int) config('saas.api.auth_token_expires', 60));
    ...
    protected function respondWithToken(string $token, $expiresAt = null): JsonResponse
    {
        return response()->json([
            'access_token' => $token,
            'token_type' => 'bearer',
            'expires_in' => config('saas.api.auth_token_expires', 60),
        ]);
    }
    ...
    {
        $minPasswordLength = max(8, (int) config('saas.auth.min_password_length', 8));

...
test('payment_method.attached creates a local payment method record', function () {
    $stripePmId = 'pm_'.uniqid();
    ...
    $pm = PaymentMethod::query()->where('stripe_id', $stripePmId)->first();

    expect($pm)->not->toBeNull()
        ->and($pm->stripe_customer_id)->toBe($this->user->stripe_id)
        ->and($pm->billable_type)->toBe('user')
        ->and($pm->billable_id)->toBe($this->user->id)
        ->and($pm->type)->toBe('card')
        ->and($pm->brand)->toBe('visa')
        ->and($pm->last4)->toBe('4242')
        ->and($pm->exp_month)->toBe(12)
        ->and($pm->exp_year)->toBe(2028)
        ->and($pm->is_default)->toBeFalse();
})->group('stripe', 'billing');

test('payment_method.updated updates an existing payment method', function () {
    $stripePmId = 'pm_'.uniqid();

    PaymentMethod::query()->create([
        'stripe_id' => $stripePmId,
        'stripe_customer_id' => $this->user->stripe_id,
        'billable_type' => 'user',
        'billable_id' => $this->user->id,
        'type' => 'card',
    ...
    expect($pm->exp_month)->toBe(6)
        ->and($pm->exp_year)->toBe(2029)
        ->and(PaymentMethod::query()->where('stripe_id', $stripePmId)->count())->toBe(1);
})->group('stripe', 'billing');

test('payment_method.detached deletes the local payment method', function () {
    $stripePmId = 'pm_'.uniqid();
    ...
    expect(PaymentMethod::query()->where('stripe_id', $stripePmId)->exists())->toBeFalse();
})->group('stripe', 'billing');
...
test('customer.updated sets default payment method flag', function () {
    $defaultPmId = 'pm_default_'.uniqid();
    $otherPmId = 'pm_other_'.uniqid();

    PaymentMethod::query()->create([
        'stripe_id' => $defaultPmId,
        'stripe_customer_id' => $this->user->stripe_id,
        'billable_type' => 'user',
        'billable_id' => $this->user->id,
        'type' => 'card',
    ...
    $defaultPm = PaymentMethod::query()->where('stripe_id', $defaultPmId)->first();
    $otherPm = PaymentMethod::query()->where('stripe_id', $otherPmId)->first();

    expect($defaultPm->is_default)->toBeTrue()
        ->and($otherPm->is_default)->toBeFalse();
})->group('stripe', 'billing');
